...
# WebAssembly
RUN wget -q https://github.com/WebAssembly/wabt/releases/download/1.0.18/wabt-1.0.18-ubuntu.tar.gz
RUN tar xzf wabt-1.0.18-ubuntu.tar.gz
RUN mkdir /usr/share/wabt
RUN mv wabt-1.0.18/bin/wat2wasm /usr/share/wabt/

# Add several languages to PATH
RUN echo "export PATH=/usr/share/wabt:/root/dlang/ldc-1.22.0/bin:/usr/share/v_linux/v:/usr/share/janet:/usr/share/swift/usr/bin:$PATH" >> ~/.bashrc
